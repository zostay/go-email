...
var (
	...
	// that method is called on a MimePart that is not a branch MimePart with
	// sub-parts.
	ErrNotMultipart = errors.New("not a multipart message")

	// ErrMultipart is the error that should be returend from GetReader() when
	// that method is called on a MimePart that is not a leaf MimePart with a
	// body to be read.
	ErrMultipart = errors.New("this is a multipart message")
)

// MimePart is an interface define the parts of a MimeMessage. Each MimePart is
// either a branch or a leaf.
//
// A branch MimePart is one that has sub-parts. In this case, the IsMultipart()
// method will return true. The GetParts() method is available, but the
// GetReader() must not be called.
//
// A leaf MimePart is one that contains content. In this case, the IsMultipart()
// method will return false. The GetParts() method must not be called on a leaf
// MimePart. However, the GetReader() method will return a reader for reading
// the content of the part.
//
// It should be noted that it is possible for a MimePart to contain content that
...
type MimePart interface {
	...
	// IsMultipart will return true if this MimePart is a branch with nested
	// parts. You may call the GetParts() method to process the parts only if
	// this returns true. If it returns false, this MimePart is a leaf and it
	...
	// GetHeader is available on all MimePart objects.
	...
	GetParts() ([]MimePart, error)
}

// GenericMessage is just an alias for MimePart, which is intended to convey the
...
// returned is either a *Message or a *MimeMessage (or nil, in case of an
// error).
type GenericMessage = MimePart

// MimeMessage is a multipart MIME message. When building these methods the MIME
...
func (mm *MimeMessage) initParts() {
	if mm.parts == nil {
		mm.parts = make([]MimePart, 0, 10)
	}
}
...
// well.
func (mm *MimeMessage) WriteTo(w io.Writer) (int64, error) {
